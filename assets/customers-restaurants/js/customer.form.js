...
document.getElementById("register").addEventListener("click", (e)=> {
    ...
    if(document.getElementById("file").files.length > 0 && document.RegisterAccount.querySelectorAll(".invalid").length === 0) {
        ...
        let customer_name = document.RegisterAccount.customer_name.value.trim();
        ...
        let email = document.RegisterAccount.email.value.trim();
        let password = document.RegisterAccount.password.value.trim();

        getCityId(state_name, city_name, zip_code, function (city) {
            if(!city.error) {
                let city_id = city.id;//document.RegisterAccount.city.value.trim();

                let customerAccount = new Account(user_name, customer_name, city_id, address, contact_phone, email, password);

                let form_data = new FormData();

                for (let key in customerAccount) {
                    form_data.append(key, customerAccount[key]);
                }
                ...
                xhr.open("POST", "/customers/write");
                xhr.setRequestHeader("cache-control", "no-cache");
                xhr.setRequestHeader("postman-token", "1fe71db8-4d19-8fda-536a-0b4da14275ff");
...
document.addEventListener('focusout', function(e) {
    debugger;
    if(e.target.type === "text" || e.target.type === "password" || e.target.type === "tel") {
